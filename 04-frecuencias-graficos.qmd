...
a.  Construir la tabla de frecuencias.
    ...
    Para obtener la tabla de frecuencias podemos usar la función [`count(df, columna)`](https://aprendeconalf.es/manual-r/06-preprocesamiento.html#conteo-del-n%C3%BAmero-de-observaciones) del paquete `dplyr` de `tidyverse` para contar las frecuencias absolutas de cada valor de la columna especificada.

    Posteriormente podemos añadir nuevas columnas a la tabla de frecuencias mediante la función [`mutate(df, variable = formula)`](https://dplyr.tidyverse.org/reference/mutate.html) usando las fórmulas para calcular las frecuencias relativas (`n/sum(n)`), frecuencias absolutas acumuladas (`cumsum(n)`) y frecuencias relativas acumuladas (`cumsum(n)/sum(n)`).
    ...
    También podemos obtener la tabla de frecuencias agrupando el data frame según la columna `hijos` con la función [`group_by(df, columna)`](https://dplyr.tidyverse.org/reference/group_by.html) y después usando la función  [`summarise`(df, variable = formula)]`(https://dplyr.tidyverse.org/reference/summarise.html) usando las fórmulas para calcular las frecuencias absolutas (`n()`), frecuencias relativas (`n()/nrow(df)`), frecuencias absolutas acumuladas (`cumsum(n())`) y frecuencias relativas acumuladas (`cumsum(n())/ nrow(df)`).
